...
void DFOC_Vbus(float power_supply)
{
  ...
  Wire1.begin(23,5, 400000UL); //23 5   13  15

  // S0.Sensor_init(&S0_I2C);   //初始化编码器0
  // S1.Sensor_init(&S1_I2C);
  S0.Sensor_init(&Wire);   //初始化编码器0
  S1.Sensor_init(&Wire1);
...
float DFOC_M0_alignSensor(int _PP)
{ 
  M0_PP=_PP;
  M0_setTorque(3, 0);  //起劲
  delay(1000);
  S0.Sensor_update();  //更新角度，方便下面电角度读取
  float S0_zero_electric_angle_DIR1 = S0.getMechanicalAngle();
  ...
  S0.Sensor_update();  //更新角度，方便下面电角度读取
  float S0_zero_electric_angle_DIR2 = S0.getMechanicalAngle();
  float delta = S0_zero_electric_angle_DIR2 - S0_zero_electric_angle_DIR1;
  if(delta > PI) delta -= 6.28;
  if(delta < -PI) delta += 6.28;
  float expected_delta = (2 * PI) / _PP;
  if(abs(delta) < expected_delta * 0.5)
  {
    Serial.println("校准失败，电机没有旋转!!!");
  }
  ...
  Serial.print("M0 DIR：");Serial.println(M0_DIR);
  Serial.print("M0 0电角度：");Serial.println(S0_zero_electric_angle);
  return S0_zero_electric_angle;
}

float DFOC_M1_alignSensor(int _PP)
{ 
  M1_PP=_PP;
  M1_setTorque(3, 0);  //起劲
  delay(1000);
  S1.Sensor_update();  //更新角度，方便下面电角度读取
  float S1_zero_electric_angle_DIR1 = S1.getMechanicalAngle();
  for(float ang = 0; ang < 2*PI ; ang += 0.1)
  {
    M1_setTorque(3, ang);
    delay(10);
  ...
  S1.Sensor_update();  //更新角度，方便下面电角度读取
  float S1_zero_electric_angle_DIR2 = S1.getMechanicalAngle();
  float delta = S1_zero_electric_angle_DIR2 - S1_zero_electric_angle_DIR1;
  if(delta > PI) delta -= 6.28;
  if(delta < -PI) delta += 6.28;
  float expected_delta = (2 * PI) / _PP;
  if(abs(delta) < expected_delta * 0.5)
  ...
  Serial.print("M1 DIR：");Serial.println(M1_DIR);
  Serial.print("M1 0电角度：");Serial.println(S1_zero_electric_angle);
  return S1_zero_electric_angle;
}

void DFOC_M0_Set_alignSensor(int _PP,int _DIR,float zero_electric_angle)
{ 
  M0_PP=_PP;
  M0_DIR=_DIR;
  S0_zero_electric_angle = zero_electric_angle;
}

void DFOC_M1_Set_alignSensor(int _PP,int _DIR,float zero_electric_angle)
{ 
  M1_PP=_PP;
  M1_DIR=_DIR;
  S1_zero_electric_angle = zero_electric_angle;
}
...
void DFOC_M1_setTorque(float Target)            //电流力矩环
{
  ...
  // Serial.println(S1_electricalAngle());
}
