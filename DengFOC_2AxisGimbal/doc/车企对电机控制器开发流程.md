...
### 6.7 MATLAB/Simulink FOC MBD 模型例程

MATLAB/Simulink 提供了较完整的 FOC MBD（Model-Based Design）参考例程，主要分布在 Motor Control Blockset、Simscape Electrical 以及部分 MCU 硬件支持包中。它们适合用于算法建模、MIL/SIL 验证、自动代码生成、HIL 验证和控制参数标定。

| 例程 | 说明 |
| --- | --- |
| [Field-Oriented Control of PMSM Using SI Units](https://www.mathworks.com/help/mcb/gs/foc-pmsm-using-si-units.html) | PMSM FOC 基础模型，使用 SI 单位，适合理解算法结构 |
| [MATLAB Project for FOC of PMSM with Quadrature Encoder](https://www.mathworks.com/help/mcb/gs/matlab-project-foc-pmsm-qep.html) | 带编码器的完整 FOC 工程项目，包含运行模式、ADC offset、编码器 offset 标定 |
| [Sensorless Field-Oriented Control of PMSM](https://www.mathworks.com/help/mcb/gs/sensorless-foc-pmsm-smo-fo.html) | 无位置 FOC 例程，支持 SMO、Flux Observer、Extended EMF Observer |
| [Field-Weakening Control with MTPA of PMSM](https://www.mathworks.com/help/mcb/gs/field-weakening-control-mtpa-pmsm.html) | 弱磁 + MTPA 控制，适合高速区控制参考 |
| [PMSM Field-Oriented Control Block](https://www.mathworks.com/help/sps/ref/pmsmfieldorientedcontrol.html) | Simscape Electrical 中的 PMSM FOC 控制模块 |
| [FOC of PMSM Using HIL Simulation](https://www.mathworks.com/help/mcb/gs/field-oriented-control-pmsm-using-hardware-in-the-loop-hil-simulation.html) | HIL 验证流程，适合做控制器实时验证 |
| [FOC with Quadrature Encoder Using C2000](https://www.mathworks.com/help/ti-c2000/ug/foc-qep-example.html) | TI C2000 硬件部署例程，适合参考编码器 FOC 工程落地 |
| [FOC with Hall Sensor Using C2000](https://www.mathworks.com/help/ti-c2000/ug/foc-hall-sensor-example.html) | TI C2000 Hall 传感器 FOC 部署例程 |

#### 6.7.1 推荐学习与复用路径

```mermaid
flowchart TB
    A[基础FOC模型<br/>PMSM FOC Using SI Units] --> B[传感器FOC工程<br/>Quadrature Encoder Project]
    B --> C[无位置算法<br/>Sensorless FOC]
    C --> D[弱磁与MTPA<br/>Field Weakening + MTPA]
    D --> E[代码生成<br/>Embedded Coder]
    E --> F[SIL/PIL/HIL验证]
    F --> G[S32K3平台适配<br/>RTD/MCAL外设驱动]
```

建议优先按以下顺序使用：

1. 用 `Field-Oriented Control of PMSM Using SI Units` 理解 FOC 主链路，包括 Clarke/Park、电流环、SVPWM 和电机模型。
2. 用 `MATLAB Project for FOC of PMSM with Quadrature Encoder` 学习工程化项目结构、启动流程、ADC 偏置校准和编码器零位标定。
3. 用 `Sensorless Field-Oriented Control of PMSM` 对照前文无位置算法，重点分析 SMO、Flux Observer、Extended EMF Observer 的角度估算与切换逻辑。
4. 用 `Field-Weakening Control with MTPA of PMSM` 学习高速区弱磁、MTPA 表和电压限幅策略。
5. 如果需要实时验证，可参考 `FOC of PMSM Using HIL Simulation` 建立 MIL/SIL/HIL 闭环。

#### 6.7.2 与 S32K3 平台的关系

MathWorks 官方硬件部署例程较多面向 TI C2000 平台，通常不能直接一键部署到 NXP S32K3。对于 S32K3 项目，更现实的路线是：

| 层级 | Simulink/MBD 负责 | S32K3 工程负责 |
| --- | --- | --- |
| 算法层 | FOC、MTPA、弱磁、无位置估算器、保护限幅 | 集成生成代码，保证定点/浮点配置和实时性 |
| 模型验证 | MIL、SIL、测试向量、参数扫描 | 与台架/HIL数据对比，校正模型参数 |
| 代码生成 | Embedded Coder 生成 ANSI C 或 MISRA 风格 C | 适配编译器、链接脚本、内存段和接口封装 |
| 外设驱动 | 抽象输入输出接口 | 用 S32K3 RTD/MCAL 实现 eMIOS、BCTU、ADC、FlexCAN、SPI |
| 标定接口 | 参数表、查表模块、A2L 生成 | 与 FreeMASTER、XCP、CANape 或内部标定协议集成 |

#### 6.7.3 S32K3 适配建议

```mermaid
flowchart TB
    Model[Simulink FOC算法模型] --> Code[Embedded Coder生成C代码]
    Code --> Wrapper[平台适配层<br/>输入/输出/参数接口]
    Wrapper --> S32[S32K3应用工程]
    S32 --> RTD[S32K3 RTD/MCAL<br/>PWM/ADC/CAN/SPI/FLS]
    RTD --> HW[功率板与电机]
...
- Simulink 模型只负责算法，外设访问通过清晰接口隔离，例如 `GetPhaseCurrent()`、`GetRotorAngle()`、`SetPwmDuty()`。
- PWM、ADC、BCTU、TRGMUX、CAN-FD、SPI 等底层资源由 S32K3 RTD/MCAL 工程实现。
- 生成代码需放入固定周期任务，例如 50-100 us 电流环和 1 ms 速度/转矩环。
- 模型中使用的电机参数、PI 参数、MTPA 表、弱磁表应形成可标定数据，便于台架调整。
- 对自动生成代码做 MISRA、栈占用、执行时间和数值范围检查，确认满足车规项目要求。
- 用 MIL/SIL 先验证算法一致性，再做 HIL 和真实台架验证，避免直接上高压平台调试。
